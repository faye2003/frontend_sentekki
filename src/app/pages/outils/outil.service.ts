...
export class OutilService {
  ...
  getAllOutil(): Observable<any[]> {
    ...
    return this.http.get<{results: any[]}>(`${this.apiUrl}/outil/all/`, { headers }).pipe(
        map(response => response.results || [])
    );
    
  }

   getUserOutil(): Observable<any[]> {
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${this.authService.getAccessToken()}`
    });
    return this.http.get<{ count: number, results: any[] }>(`${this.apiUrl}/outil/recent/`, { headers }).pipe(
